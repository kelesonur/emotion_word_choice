...
?bayes.cor.test
bayes.cor
dfjapan <- read_excel("culture_japan.xlsx" ) %>% drop_na()
...
cor.test(df_nomix$arousal, df_nomix$idv, method = "pearson")
...
df2$mixed %<>% as.factor() 
levels(df2_nomix$Emotion_dyads)
levels(df2_nomix$Main_emotion)
...
df2_nomix2 <- df2 %>% subset(arousal != "Mixed or no emotion")
...
df2$arousal2 <- df2$arousal %>% as.character() 
  View(df2_nomix)
...
# creating a df for the model results with the mcmc_intervals_data function. I included only the predictor estimate. SPECIAL
df_m1 <- mcmc_intervals_data(arousal.model.nomix, pars = vars(starts_with("b_"))) %>% subset(parameter != "b_Intercept")
...
# plotting the model plot with credible intervals, and a dashed vertical line at the intercept (0). SPECIAL. Modele plot yapmak zorundna deðilsin
p4 <- df_m1 %>% ggplot(aes(m, parameter)) +
  geom_point(size = 3) +
  geom_errorbarh(aes(xmin = l, xmax = h), alpha = 1, height = 0, size = 2) +
  geom_errorbarh(aes(xmin = ll, xmax = hh, height = 0)) + 
  geom_vline(xintercept = 0, linetype = "dashed", size = 1, colour = "black") +
